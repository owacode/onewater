...
export class AuthorPageComponent implements OnInit {
  ...
  ngOnInit() {
    this.router.params.subscribe(result=> {
      //console.log(result)
      this.http.get<{status:string, msg:string, result:any}>('http://18.223.24.178:8000/approvedauthor/'+result.id)
      .subscribe(result=>{
        //console.log(result);
  ...
  getblogs(id){
    this.http.get<{status:string, msg:string, result:any}>('http://18.223.24.178:8000/authorapprovedblogs/'+id)
    .subscribe(result=>{
